...
log10_read_filtration_barplot <- read_filtration_barplot + scale_y_continuous(
  trans='log10'
)
...
if (!file.exists(paste0(Robject_dir, "all_fusion_reads.Rdata"))) {
  ...
  for (i in 1:length(fusions)) {
    ...
    spanning_supporting_reads <- lapply(
      ...
      chr11_fusions[[i]], chr22_fusions[[i]], disc_read_window
    )
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
  final_venns$high_conf
```

Final VAF = **`r VAFs$high_conf`**% \

Low confidence fusions: \

```{r, echo = FALSE, message = FALSE, warning = FALSE}
  final_venns$low_conf
```

Final VAF = **`r VAFs$low_conf`**% \
