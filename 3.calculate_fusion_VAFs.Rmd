...
rmarkdown::render(
  ...
  params = list(samplename = "409_002_D9YW9_GGACTCCT-CTCTCTAT_L001"),
  output_file="/share/ScratchGeneral/jamtor/projects/ewing_ctDNA/results/VAF_calculation/409_002_D9YW9_GGACTCCT-CTCTCTAT_L001/VAF_calculation_report.html"
)
-->
...
```{r, create_venns, include = FALSE}
  ...
  log10_read_filtration_barplot <- p + ylab("No. reads (log10)")
...
```{r, calculate_VAFs, include = FALSE}
  ...
  if (!is.na(all_reads)) {
    ...
    VAF <- data.frame(VAF)
    colnames(VAF) <- names(which.max(read_counts["supporting",]))
...
```{r, fig.align = "center", out.width = "8.5in", echo = FALSE, message = FALSE, warning = FALSE}
  final_venn
```
